...
Public Class carteraAd
    ...
    End Function
    '****************************************************************************************************************
    'Funcion encargada de listar los cheques devueltos****************************************************************
    '****************************************************************************************************************
    Public Function listar_cheq_dev(ByVal nit As Double, ByVal fec As String) As DataTable
        Dim sql As String
        conn = objConexion.abrirConexion
        Dim dt As New DataTable
        sql = "SELECT  tipo, numero, nit, fecha, valor_total, notas " & _
                                            "FROM documentos " & _
                                                    "WHERE     (tipo IN ('NDCD', 'NDCH')) and nit = " & nit & " and fecha > = '" & fec & "' "
    ...
    Public Function sumar_cheq_dev(ByVal nit As Double) As Double
        ...
        Dim sFec As String = fec.Year & "-" & fec.Month & "-" & fec.Day
        Dim reader As SqlDataReader
        Dim sql As String
        conn = objConexion.abrirConexion
        comando.Connection = conn
        sql = "SELECT  SUM (valor_total ) " & _
                                            "FROM documentos " & _
                                                    "WHERE   nit = " & nit & " and  (tipo IN ('NDCD', 'NDCH')) and fecha > ='" & sFec & "'"
        comando.CommandType = CommandType.Text
    ...
    Public Function sumarSaldo(ByVal nit As Double) As Double
        ...
        comando.Connection = conn
        comando.CommandType = CommandType.Text
        sql = "SELECT  ROUND(SUM(Saldo),0) FROM V_cartera_edades_jjv where nit=" & nit & "and saldo <> 0"
        comando.CommandText = (sql)
        reader = comando.ExecuteReader
        While reader.Read
            If IsDBNull(reader(0)) Then
