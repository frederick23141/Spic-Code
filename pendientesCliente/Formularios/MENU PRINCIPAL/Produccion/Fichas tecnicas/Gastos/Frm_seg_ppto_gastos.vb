...
Public Class Frm_seg_ppto_gastos
    ...
    Private Function armar_dt() As DataTable
        ...
        End If
        Dim sql As String = "SELECT p.mes,p.ano,t.descripcion As tipo,c.cuenta ,c.descripcion As desc_cuenta ,(SELECT SUM (ppto) FROM J_ppto_gastos WHERE cuenta = c.cuenta  AND p.mes = mes AND ano =  p.ano " & where_centro & " ) As presupuesto ,  (SELECT SUM (valor) FROM movimiento WHERE cuenta = c.cuenta AND Year(fec) = p.ano AND MONTH(fec) = p.mes " & where_centro & " ) As saldo ,(SELECT SUM (ppto) FROM J_ppto_gastos WHERE cuenta = c.cuenta  AND ano =  p.ano " & where_centro & " ) As acum_ppto, (SELECT SUM (valor) FROM movimiento WHERE cuenta = c.cuenta AND Year(fec) = p.ano  " & where_centro & "  ) As acum_saldo " & _
    ...
    Private Sub formatoDtg()
        ...
        If dtg_consulta.Rows.Count > 0 Then
            ...
            For j = 0 To dtg_consulta.Columns.Count - 1
                ...
                If (nom_col = "porc_tend_hoy" Or nom_col = "porc_tend_mes" Or nom_col = "porc_cump" Or nom_col = "cuenta" Or nom_col = "diferencia") Then
    ...
    Private Sub totalizar_tipo_producto(ByRef dt As DataTable)
        Dim sum As Double = 0
        For j = 0 To dt.Columns.Count - 1
            If (dt.Columns(j).ColumnName = "presupuesto" Or dt.Columns(j).ColumnName = "saldo" Or dt.Columns(j).ColumnName = "porc_cump" Or dt.Columns(j).ColumnName = "diferencia" Or dt.Columns(j).ColumnName = "acum_ppto" Or dt.Columns(j).ColumnName = "acum_saldo") Then
                For i = 0 To dt.Rows.Count - 2
