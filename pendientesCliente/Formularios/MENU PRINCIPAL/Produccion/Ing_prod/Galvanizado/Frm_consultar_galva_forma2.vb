...
Public Class Frm_consultar_galva_forma2
    ...
    Private Sub Frm_consultar_galva_forma2_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Else
            sql = "SELECT  SUM (prod.paro1 )As paro1,SUM (prod.paro2 )As paro2,SUM (prod.paro3 )As paro3,SUM (prod.paro4 )As paro4,SUM (prod.paro5 )As paro5,SUM (prod.paro6 )As paro6,SUM (prod.paro7 )As paro7, " & _
                                     "SUM(prod.paro8)As paro8 , SUM(prod.paro9)As paro9 " & _
                                         "FROM J_prod_galv_det prod,J_turnos tur " & _
    ...
    Public Sub graficar()
        If (cbo_turno.Text <> "Seleccione") Then
            ...
            Dim select_from As String = "SELECT SUM (kilos_prod) As Kilos ,DAY (fecha )" & _
                                        "FROM J_prod_galv_det d FROM J_prod_galv_enc e "
            ...
            End If
            where_sql = " WHERE e.id = d.id_enc AND fecha>= '" & fec_ini & "' AND fecha<= '" & fec_fin & "' " & criterio
    ...
    Private Sub EliminarToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles EliminarToolStripMenuItem.Click
        ...
        If (pos <> dtg_consulta.RowCount - 1) Then
            ...
            Dim sql As String = "DELETE FROM J_prod_galv_enc WHERE id = " & id
            list_sql.Add(sql)
            sql = "DELETE FROM J_prod_galv_det WHERE id_enc = " & id
            list_sql.Add(sql)
    ...
    Public Sub cargar_consulta()
        If (cbo_turno.Text <> "Seleccione") Then
            ...
            If (chk_consol_ref.Checked) Then
                select_sql = "SELECT cal.descripcion As referencia,d.descripcion As destino,t.descripcion As tipo,SUM(e.horas_trab * 60) as Min_trab,SUM(tur.Horas * 60)As min_turno,SUM(det.kilos_prod)As kilos_prod "
                groupSql &= ",d.descripcion,t.descripcion "
            ElseIf (chk_consol_tur.Checked) Then
                select_sql = "SELECT cal.descripcion As referencia,tur.descripcion as Turno,d.descripcion As destino,t.descripcion As tipo,SUM(e.horas_trab * 60) as Min_trab,SUM(tur.Horas * 60)As min_turno,SUM(det.kilos_prod)As kilos_prod "
                groupSql &= ",tur.descripcion,d.descripcion,t.descripcion  "
            ElseIf (chkConsolDia.Checked) Then
                select_sql = "SELECT cal.descripcion  As referencia,e.fecha ,d.descripcion As destino,t.descripcion As tipo,SUM(e.horas_trab * 60) as Min_trab,SUM(tur.Horas * 60)As min_turno,SUM(det.kilos_prod)As kilos_prod "
                groupSql &= ", e.fecha,d.descripcion,t.descripcion  "
            Else
                select_sql = "SELECT e.id,e.fecha,horas_trab, cal.descripcion As referencia,d.descripcion As destino,t.descripcion As tipo,(e.horas_trab * 60) as Min_trab,(tur.Horas * 60)As min_turno,(det.kilos_prod)As kilos_prod "
                groupSql = ""
            End If
            Dim criterio As String = "WHERE t.id = det.tipo AND det.id_enc = e.id AND d.id = det.destino AND det.calibre = cal.id AND tur.codigo = e.turno AND e.fecha>= '" & sFec_ini & "' AND e.fecha<= '" & sFec_fin & "' "
            from_sql = " FROM j_turnos tur,j_prod_galv_enc e ,j_prod_galv_det det ,j_calibres cal,J_destino_galv d,J_prod_galv_tipo t "
            If (cbo_turno.Text <> "TODOS") Then
                criterio += " AND eturno = " & cbo_turno.SelectedValue & " "
