...
Public Class Frm_consultar_salida_materia_prima
    ...
    Private Sub Frm_consultar_salida_materia_prima_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If (e.RowIndex >= 0) Then
            ...
            If (col = colVer.Name) Then
                ...
                Dim sql As String = "SELECT l.tipo,l.numero,l.cantidad,l.fec " & _
                                        "FROM J_salida_materia_prima_G_transaccion t , CORSAN.dbo.documentos_lin l   " & _
                                          "WHERE  bodega = 1 AND l.tipo = t.tipo AND l.numero = t.num_transaccion AND t.numero = " & numero & "  AND t.id_detalle = " & id_detalle & ""
            ...
            ElseIf (col = colBorrar.Name) Then
                ...
                If (motivo <> "") Then
                    ...
                    sql = "UPDATE J_salida_materia_prima_G_enc SET anulado = 1,observaciones += '" & motivo & "' WHERE numero =" & numero
                    listSql.Add(sql)
    ...
    Private Sub cargarConsulta()
        ...
        Dim whereSql As String = "WHERE  E.anulado is null AND E.fecha >= '" & fecIni & "' AND E.fecha <= '" & fecFin & "' "
        ...
        End If
        selectSql = "SELECT E.numero,D.id_detalle,E.fecha,D.codigo,D.cantidad,(SELECT CASE WHEN (SELECT sum(peso) FROM J_salida_materia_prima_G_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle ) is null THEN 0 ELSE D.cantidad-(D.cantidad -(SELECT sum(peso) FROM J_salida_materia_prima_G_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle )) END ) As entregado_kilos,E.solicitante,E.observaciones " & _
                       "FROM J_salida_materia_prima_G_enc E ,J_salida_materia_prima_G_det D "
        ...
        If Not (chkTerminados.Checked) Then
            whereSql &= " AND (SELECT CASE WHEN (SELECT sum(peso) FROM J_salida_materia_prima_G_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle ) is null THEN 0 ELSE (D.cantidad -(SELECT sum(peso) FROM J_salida_materia_prima_G_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle )) END )<D.cantidad "
        End If
    ...
    Private Sub btnNuevo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNuevo.Click
        ...
        frm.Show()
        frm.Main(usuario, 0, "nod_salida_materia_prima")
    End Sub
