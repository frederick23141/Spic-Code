...
Public Class Frm_transaciones_materia_prima_G_devolver
    ...
    Private Sub guardar()
        Using New Centered_MessageBox(Me)
            ...
            If peso < stock Then
                ...
                Dim sql As String = "update " & db_produccion & "J_rollos_tref set traslado=null, destino = null where  id_detalle =" & id_detalle & "  AND id_rollo =" & id_rollo & "and cod_orden=" & consecutivo_materia_prima
                listSql.Add(sql)
                                        ...
                                        If (obj_Ing_prodLn.existeTipoTransaccion(cboTipo.Text)) Then
                                            If txtCodigoLector.Text <> "" Then
                                                If validarCodigoBarras(txtCodigoLector.Text) Then
                                                    Return True
                                                Else
                                                    MessageBox.Show("El código de barras no se encuentra asignado o no hace parte de galvanizado", "Código de barras no asignado", MessageBoxButtons.OK, MessageBoxIcon.Error)
                                                End If
                                            Else
    ...
    Private Function validarTraslado(ByVal consecutivo As String) As Boolean
        ...
        If (objOperacionesDb.consultValor(sqlTraslado, "PRODUCCION").ToUpper <> "G") Then
            resp = False
        End If
