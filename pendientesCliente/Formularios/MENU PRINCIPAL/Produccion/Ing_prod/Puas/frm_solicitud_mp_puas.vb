...
Public Class frm_solicitud_mp_puas
    ...
    Private Sub guardar()
        ...
        End If
        'If (numeroExistente = 0) Then
        '    numero = objIngresoProdLn.consultar_valor("SELECT MAX (numero)  FROM J_salida_materia_prima_PU_enc ", "PRODUCCION") + 1
        'Else
        '    numero = numeroExistente
        'End If
        'sqlDet = "DELETE FROM J_salida_materia_prima_PU_det WHERE numero =" & numero
        'listSql.Add(sqlDet)
        'sqlEnc = "DELETE FROM J_salida_materia_prima_PU_enc WHERE numero =" & numero
        'listSql.Add(sqlEnc)
        'sqlEnc = "INSERT INTO J_salida_materia_prima_PU_enc (numero,fecha,solicitante,observaciones,devolver) " & _
        ...
        For i = 0 To dtg_solicitud.RowCount - 1
            If (dtg_solicitud.Item(colDesc.Name, i).Value <> "") Then
                ...
                'sqlDet = "INSERT J_salida_materia_prima_PU_det (numero,id_detalle,codigo,cantidad) VALUES " & _
    ...
    Public Sub guardar_automatico(ByVal cod_rec As String, ByVal cantidad As String, ByVal mes As Integer, ByVal year As Integer, ByVal solicitante As String, ByVal observaciones As String, ByVal devolver As String)
        ...
        Dim sqlEnc As String = ""
        Dim medida As String = ""
        Dim sql As String = "SELECT max(d.numero) FROM J_salida_materia_prima_PU_enc e JOIN J_salida_materia_prima_PU_det d on e.numero=d.numero" &
        ...
        If existe_orden = "" Then
            ...
            sqlEnc = "DELETE FROM J_salida_materia_prima_PU_enc WHERE numero =" & numero
            listSql.Add(sqlEnc)
            sqlEnc = "INSERT INTO J_salida_materia_prima_PU_enc (numero,fecha,solicitante,observaciones,devolver) " &
                                "VALUES (" & numero & ",'" & fecha_unida & "'," & solicitante & ",'" & observaciones & "','" & devolver & "')"
            listSql.Add(sqlEnc)
            sqlDet = "INSERT J_salida_materia_prima_PU_det (numero,id_detalle,codigo,cantidad) VALUES " &
                                   "(" & numero & "," & "1" & ",'" & cod_rec & "'," & cantidad & ")"
            listSql.Add(sqlDet)
        Else
            sqlDet = "UPDATE J_salida_materia_prima_PU_det set cantidad=cantidad+" & cantidad & " where numero=" & existe_orden & ""
            listSql.Add(sqlDet)
        End If
        insertoCorrecto = objIngresoProdLn.ExecuteSqlTransaction(listSql, "PRODUCCION")
        If (insertoCorrecto) Then
    ...
    Private Sub cargarCodigos(ByVal codigo As String, ByVal descripcion As String)
        ...
        End If
        Dim sql As String = "SELECT codigo,descripcion,costo_unitario,generico FROM referencias WHERE " & where_sql & " AND (codigo like '22G%' AND ref_anulada = 'N')"
