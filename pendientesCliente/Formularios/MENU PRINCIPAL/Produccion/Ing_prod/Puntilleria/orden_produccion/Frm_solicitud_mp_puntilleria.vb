...
Public Class Frm_solicitud_mp_puntilleria
    ...
    Public Sub Main(ByVal usuario As UsuarioEn, ByVal numeroExistente As Integer)
        ...
        carga_comp = True
    End Sub
    Private Sub Frm_solicitud_mp_puntilleria_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        cargar_cbo()
    ...
    Private Sub guardar()
        ...
        'If (numeroExistente = 0) Then
        '    numero = objIngresoProdLn.consultar_valor("SELECT MAX (numero)  FROM J_salida_materia_prima_P_enc ", "PRODUCCION") + 1
        'Else
        '    numero = numeroExistente
        'End If
        'sqlDet = "DELETE FROM J_salida_materia_prima_P_det WHERE numero =" & numero
        'listSql.Add(sqlDet)
        'sqlEnc = "DELETE FROM J_salida_materia_prima_P_enc WHERE numero =" & numero
        'listSql.Add(sqlEnc)

        If chk_devolucion.Checked Then
            devolver = "S"
        End If
        'sqlEnc = "INSERT INTO J_salida_materia_prima_P_enc (numero,fecha,solicitante,observaciones,devolver) " & _
        ...
        For i = 0 To dtgSolicitud.RowCount - 1
            If (dtgSolicitud.Item(colDesc.Name, i).Value <> "") Then
                ...
                'sqlDet = "INSERT J_salida_materia_prima_P_det (numero,id_detalle,codigo,cantidad) VALUES " & _
    ...
    Public Sub guardar_automatico(ByVal cod_rec As String, ByVal cantidad As String, ByVal mes As Integer, ByVal year As Integer, ByVal solicitante As String, ByVal observaciones As String, ByVal devolver As String)
        ...
        Dim sql As String
        If devolver = "N" Then
            sql = "SELECT max(d.numero) FROM J_salida_materia_prima_P_enc e JOIN J_salida_materia_prima_P_det d on e.numero=d.numero" &
        ...
        If existe_orden = "" Then
            ...
            sqlEnc = "DELETE FROM J_salida_materia_prima_P_enc WHERE numero =" & numero
            listSql.Add(sqlEnc)
            sqlEnc = "INSERT INTO J_salida_materia_prima_P_enc (numero,fecha,solicitante,observaciones,devolver) " &
                                "VALUES (" & numero & ",'" & fecha_unida & "'," & solicitante & ",'" & observaciones & "','" & devolver & "')"
            listSql.Add(sqlEnc)
            sqlDet = "INSERT INTO J_salida_materia_prima_P_det (numero,id_detalle,codigo,cantidad) VALUES " &
                                   "(" & numero & "," & "1" & ",'" & cod_rec & "'," & cantidad & ")"
            listSql.Add(sqlDet)
        Else
            sqlDet = "UPDATE J_salida_materia_prima_P_det set cantidad=cantidad+" & cantidad & " where numero=" & existe_orden & ""
            listSql.Add(sqlDet)
        End If
        insertoCorrecto = objIngresoProdLn.ExecuteSqlTransaction(listSql, "PRODUCCION")
        If (insertoCorrecto) Then
    ...
    Private Sub dtgCodigo_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dtgCodigo.CellClick
        If (e.RowIndex >= 0) Then
            ...
            If chk_devolucion.Checked Then
                stock_b2 = objOpSimplesLn.consultarStock(codigo, 12)
            End If
    ...
    Private Sub cargarCodigos(ByVal codigo As String, ByVal descripcion As String)
        ...
        End If
        Dim sql As String = "SELECT codigo,descripcion,costo_unitario,generico FROM referencias WHERE " & where_sql & " AND (codigo like '22B%'  or codigo like '22G%' or codigo like '22X%' or codigo like '22R%')"
