...
Imports accesoDatos
Public Class frm_orden_prod_punt_hornos
    ...
    Private Sub btn_transaccion_empaque_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_transaccion_empaque.Click
        ...
        If (validarEmpaque()) Then
            ...
            Dim resp As Integer = MessageBox.Show("¿Desea Enviar el contenedor a Hornos?", "Enviar a Hornos", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question)
            If resp = 6 Then
                If val_punt.validarCodigoBarrasPuntilleria(consecutivo) Then
                    If val_punt.validarTransaccionEmpaque(consecutivo) Then
                        Application.DoEvents()
                        realizar_transaccion_empaque()
                        Application.DoEvents()
                    End If
                End If
            Else
        ...
        End If
    End Sub
    Private Sub realizar_transaccion_empaque()
        Using New Centered_MessageBox(Me)
            ...
            If peso <= stock Then
                ...
                Else
                    modelo = "05"
                End If
                sql = "SELECT clase FROM J_orden_prod_punt_refs_clase WHERE REF = '" & codigo.Trim & "'"
                ...
                Dim db_produccion As String = objOpsimpesLn.get_nom_db("PRODUCCION") & ".dbo."
                If clase = 2 Then
                    sql = "UPDATE " & db_produccion & "J_orden_prod_punt_producto SET sppp=" & numero_transaccion & ", hornos ='S' WHERE  nro_orden =" & orden_produccion & "  AND consecutivo_rollo =" & id_rollo
                    'listTransaccion_corsan.Add(sql)
                Else
                    sql = "UPDATE " & db_produccion & "J_orden_prod_punt_producto SET sppp=" & numero_transaccion & ", hornos ='B' WHERE  nro_orden =" & orden_produccion & "  AND consecutivo_rollo =" & id_rollo
                    'listTransaccion_corsan.Add(sql)
                End If
        ...
        End Using
    End Sub
    Private Function transaccion(ByVal datacodigo As DataTable, ByVal tipo As String, ByVal bodega As Integer, ByVal modelo As String, ByVal roll As String) As List(Of Object)
        Dim listSql As New List(Of Object)
        Dim dFec As Date = Now
        Dim usuario As String = "Puntilleria"
        Dim notas As String = "SPIC fecha:" & Now & "(" & roll & ")"
                                    ...
                                    If (obj_Ing_prodLn.existeTipoTransaccion(cbo_transaccion_empaque.Text)) Then
                                        If txt_lector_empaque.Text <> "" Then
                                            If val_punt.validarCodigoBarrasPuntilleria(txt_lector_empaque.Text) Then
                                                If val_punt.validar_horno(txt_lector_empaque.Text, lblCodigoEmpaque.Text) Then
                                                    Return True
                                                End If
                                            Else
