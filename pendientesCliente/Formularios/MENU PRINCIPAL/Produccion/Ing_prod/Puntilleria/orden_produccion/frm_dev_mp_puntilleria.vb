...
Public Class frm_dev_mp_puntilleria
        ...
        Using New Centered_MessageBox(Me)
            ...
            If peso < stock Then
                ...
                If codigo_aut.ToUpper Like "22G*" Then
                    ...
                    sql = "UPDATE " & db_produccion & "D_rollo_galvanizado_f SET destino = NULL, traslado=NULL where  nro_orden =" & consecutivo_materia_prima & "  AND consecutivo_rollo =" & id_rollo
                    listSql.Add(sql)
                Else
                    ...
                    sql = "update " & db_produccion & "J_rollos_tref set traslado=NULL,destino=NULL where  id_detalle =" & id_detalle & "  AND id_rollo =" & id_rollo & "and cod_orden=" & consecutivo_materia_prima & ""
                    listSql.Add(sql)
                End If
                sql = "INSERT INTO " & db_produccion & "J_salida_materia_prima_P_transaccion" & _
                ...
                Else
                    MessageBox.Show("Error al realizar la transaccion. ", tipo & ": " & Convert.ToString(numero_transaccion), MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
            Else
                                            ...
                                            If txtCodigoLector.Text <> "" Then
                                                ...
                                                Else
                                                    If validarCodigoBarras(txtCodigoLector.Text) Then
                                                        Return True
                                                    Else
                                                        MessageBox.Show("El código de barras no se encuentra asignado o no se encuentra en bodega 12", "Código de barras no asignado", MessageBoxButtons.OK, MessageBoxIcon.Error)
                                                    End If
                                                End If
                                            Else
    ...
    Private Function validar_guardado_rollo(ByVal peso As String)
        ...
        Dim peso_solicitud As Integer = 0
    ...
    Private Function validarTrasladoGalv(ByVal consecutivo As String) As Boolean
        ...
        If (objOperacionesDb.consultValor(sqlTraslado, "PRODUCCION").ToUpper <> "P") Then
            resp = False
        End If
        sqlTraslado = "SELECT anulado FROM D_rollo_galvanizado_f  WHERE nro_orden =" & nro_orden & " AND consecutivo_rollo = " & id_rollo
        If (objOperacionesDb.consultValor(sqlTraslado, "PRODUCCION").ToUpper <> "") Then
            resp = False
        End If
        Return resp
    End Function
