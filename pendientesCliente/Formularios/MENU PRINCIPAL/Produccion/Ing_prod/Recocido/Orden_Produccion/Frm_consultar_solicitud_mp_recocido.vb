...
Public Class Frm_consultar_solicitud_mp_recocido
    ...
    Private Sub cargar_cbo()
        ...
        Dim dr As DataRow
        dt = New DataTable
        sql = "SELECT nit,nombres FROM V_nom_personal_Activo_con_maquila where centro in (2200) order by nombres"
        ...
        If (e.RowIndex >= 0) Then
            ...
            If (col = colVer.Name) Then
                ...
                Dim sql As String = "SELECT l.tipo,l.numero,l.cantidad,l.fec " & _
                                        "FROM J_salida_materia_prima_R_transaccion t , CORSAN.dbo.documentos_lin l   " & _
                                          "WHERE  l.bodega = '13' AND l.tipo = t.tipo AND l.numero = t.num_transaccion AND t.numero = " & numero & "  AND t.id_detalle = " & id_detalle & ""
            ...
            ElseIf (col = colBorrar.Name) Then
                ...
                If (motivo <> "") Then
                    ...
                    sql = "UPDATE J_salida_materia_prima_R_enc SET anulado = 1,observaciones += '" & motivo & "' WHERE numero =" & numero
                    listSql.Add(sql)
    ...
    Private Sub cargarConsulta()
        ...
        End If
        selectSql = "SELECT E.numero,D.id_detalle,E.fecha,D.codigo,D.cantidad,(SELECT CASE WHEN (SELECT sum(peso) FROM J_salida_materia_prima_R_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle ) is null THEN 0 ELSE D.cantidad-(D.cantidad -(SELECT sum(peso) FROM J_salida_materia_prima_R_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle )) END ) As entregado_kilos,E.solicitante,t.nombres,E.observaciones " &
                       "FROM J_salida_materia_prima_R_enc E ,J_salida_materia_prima_R_det D, CORSAN.dbo.V_nom_personal_Activo_con_maquila_general t "
        ...
        If Not (chkTerminados.Checked) Then
            whereSql &= " AND (SELECT CASE WHEN (SELECT sum(peso) FROM J_salida_materia_prima_R_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle ) is null THEN 0 ELSE (D.cantidad -(SELECT sum(peso) FROM J_salida_materia_prima_R_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle )) END )<D.cantidad "
        End If
