...
Imports accesoDatos
Public Class Frm_transacion_mp_recocido
    ...
    Public Sub Main(ByVal nit As Double, ByVal bod_origen As Integer, ByVal bod_destino As Integer, ByVal modelo As String)
        ...
        Me.Text = "Traslado de bodega:" & bod_origen & " - " & bod_destino
    End Sub

    Private Sub Frm_transacion_mp_recocido_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub cargar_solicitudes()
        ...
        If bod_origen = 13 And bod_destino = 2 Then
            whereSql &= " AND e.devolver = 'S' "
        Else
            whereSql &= " AND (e.devolver = 'N' OR e.devolver IS NULL ) "
        End If
        Dim sql As String = "SELECT E.numero,D.id_detalle,E.fecha,D.codigo,(SELECT CASE WHEN (SELECT sum(peso) FROM J_salida_materia_prima_R_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle ) is null THEN D.cantidad  ELSE (D.cantidad -(SELECT sum(peso) FROM J_salida_materia_prima_R_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle )) END )As pendiente  ,R.descripcion FROM J_salida_materia_prima_R_enc E ,J_salida_materia_prima_R_det D, CORSAN.dbo.referencias R " & _
        " WHERE year(E.fecha)=" & Now.Year & " AND E.anulado is null  AND  R.codigo = D.codigo AND D.numero = E.numero " & whereSql & _
        "AND (D.cantidad - (SELECT CASE WHEN ((SELECT sum(peso) FROM J_salida_materia_prima_R_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle )) is null THEN 0 ELSE ((SELECT sum(peso) FROM J_salida_materia_prima_R_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle ))END) > 0 ) ORDER BY E.fecha "
    ...
    Private Sub dtg_pedido_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dtg_pedido.CellContentClick
        If (e.RowIndex >= 0) Then
            ...
            If (col = colVer.Name) Then
                ...
                Me.Text = codigo_aut & "-Traslado de bodega:" & bod_origen & "-" & bod_destino
    ...
    Private Sub guardar()
        Using New Centered_MessageBox(Me)
            ...
            If peso <= stock Then
                ...
                'Se inserto el cambo "db_produccion" para que en SQL de la insersion y el update se pueda añadir a un solo listSql el cual
                'nos va a evitar errores en la parte de la transaccion.
                'Dentro del update se inserto la base de datos de produccion utilizada, para evitar errores con la base de datos de prueba.

                Dim sql As String


                If bodega = 13 Then
                    sql = "UPDATE " & db_produccion & "J_rollos_tref SET destino = null, traslado=null where  id_detalle =" & id_detalle & "  AND id_rollo =" & id_rollo & " and cod_orden=" & consecutivo_materia_prima & ""
                    listSql.Add(sql)
                    sql = "UPDATE " & db_produccion & "J_rollos_tref SET traslado_D=" & numero_transaccion & " where  id_detalle =" & id_detalle & "  AND id_rollo =" & id_rollo & " and cod_orden=" & consecutivo_materia_prima & ""
                    listSql.Add(sql)
                ElseIf bodega = 2 Then
                    sql = "UPDATE " & db_produccion & "J_rollos_tref SET destino = 'R', traslado=" & numero_transaccion & " where  id_detalle =" & id_detalle & "  AND id_rollo =" & id_rollo & " and cod_orden=" & consecutivo_materia_prima & ""
                    listSql.Add(sql)
                End If
                sql = "INSERT INTO " & db_produccion & "J_salida_materia_prima_R_transaccion" & _
                                        ...
                                        If (obj_Ing_prodLn.existeTipoTransaccion(cboTipo.Text)) Then
                                            If txtCodigoLector.Text <> "" Then
                                                If validarCodigoBarras(txtCodigoLector.Text) Then
                                                    Return True
                                                Else
                                                    MessageBox.Show("El código de barras no se encuentra asignado o el rollo no esta en la bodega 13", "Código de barras no asignado", MessageBoxButtons.OK, MessageBoxIcon.Error)
                                                End If
                                            Else
    ...
    Private Function validarCodigoBarras(ByVal consecutivo As String) As Boolean
        ...
        If consecutivo_materia_prima <> "" And id_rollo <> "" And id_detalle <> "" Then
            Dim sql As String = "select destino from J_rollos_tref WHERE cod_orden =" & consecutivo_materia_prima & " AND id_detalle = " & id_detalle & " AND id_rollo = " & id_rollo
            Dim id As String = objOpSimplesLn.consultValorTodo(sql, "PRODUCCION")
            If id <> "" Then
                If bod_origen = 13 Then
                    If id = "R" Then
                        resp = True
                    End If
                End If
            Else
                If bod_origen = 2 Then
                    sql = "select cod_orden from J_rollos_tref WHERE cod_orden =" & consecutivo_materia_prima & " AND id_detalle = " & id_detalle & " AND id_rollo = " & id_rollo
        ...
        If resp = False Then
            Using New Centered_MessageBox(Me)
                If bod_origen = 2 Then
                    MessageBox.Show("El tiquete a sido trasladado o no existe", "Problemas con el tiquete", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Else
    ...
    Private Function validar_guardado_rollo(ByVal peso As String)
        ...
        If bod_origen = 13 Then
            sql_rollo = "SELECT (SELECT CASE WHEN (SELECT sum(peso) FROM J_salida_materia_prima_R_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle ) is null THEN D.cantidad  ELSE (D.cantidad -(SELECT sum(peso) FROM J_salida_materia_prima_R_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle )) END )As pendiente FROM J_salida_materia_prima_R_enc E ,J_salida_materia_prima_R_det D, CORSAN.dbo.referencias R where E.numero=" & num_solicitud & " AND E.anulado is null AND  R.codigo = D.codigo AND (e.devolver = 'N' OR e.devolver='S' ) AND E.numero = D.numero AND id_detalle = D.id_detalle"
        ElseIf bod_origen = 2 Then
            sql_rollo = "SELECT (SELECT CASE WHEN (SELECT sum(peso) FROM J_salida_materia_prima_R_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle ) is null THEN D.cantidad  ELSE (D.cantidad -(SELECT sum(peso) FROM J_salida_materia_prima_R_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle )) END )As pendiente FROM J_salida_materia_prima_R_enc E ,J_salida_materia_prima_R_det D, CORSAN.dbo.referencias R where E.numero=" & num_solicitud & " AND E.anulado is null AND  R.codigo = D.codigo AND (e.devolver = 'N' OR e.devolver IS NULL ) AND E.numero = D.numero AND id_detalle = D.id_detalle"
        End If
#Disable Warning BC42104 ' La variable 'sql_rollo' se usa antes de que se le haya asignado un valor. Podría darse una excepción de referencia NULL en tiempo de ejecución.
        peso_solicitud = CInt(objOperacionesDb.consultValor(sql_rollo, "PRODUCCION"))
#Enable Warning BC42104 ' La variable 'sql_rollo' se usa antes de que se le haya asignado un valor. Podría darse una excepción de referencia NULL en tiempo de ejecución.
    ...
    Private Function validarTraslado(ByVal consecutivo As String) As Boolean
                ...
                If IsNumeric(dt_validacion.Rows(i).Item("traslado")) Then
                    If Not IsDBNull(dt_validacion.Rows(i).Item("destino")) Then
                        If dt_validacion.Rows(i).Item("destino") = "G" Then
                            Using New Centered_MessageBox(Me)
                                MessageBox.Show("El rollo pertenece a galvanizado ", "Rollo trasladado", MessageBoxButtons.OK, MessageBoxIcon.Error)
                            End Using
                        End If
                        If dt_validacion.Rows(i).Item("destino") = "R" Then
                            Using New Centered_MessageBox(Me)
                                MessageBox.Show("El rollo pertenece a recocido ", "Rollo trasladado", MessageBoxButtons.OK, MessageBoxIcon.Error)
                            End Using
                        End If
                        If dt_validacion.Rows(i).Item("destino") = "P" Then
                            Using New Centered_MessageBox(Me)
                                MessageBox.Show("El rollo pertenece a puntilleria ", "Rollo trasladado", MessageBoxButtons.OK, MessageBoxIcon.Error)
                            End Using
                        End If
                    End If
                Else
                    ...
                    Else
                        Using New Centered_MessageBox(Me)
                            MessageBox.Show("La planilla del rollo que intenta trasladar no ha sido cerrada ", "Planilla cerrada", MessageBoxButtons.OK, MessageBoxIcon.Error)
                        End Using
                    End If
                End If
            Next
        Else
            For i = 0 To dt_validacion.Rows.Count - 1
                If dt_validacion.Rows(i).Item("destino") <> "R" And IsNumeric(dt_validacion.Rows(i).Item("traslado")) Then
                    Using New Centered_MessageBox(Me)
                        MessageBox.Show("El rollo no pertenece a recocido ", "Rollo trasladado", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    End Using
                Else
                    resp = True
                End If
