...
Public Class Frm_consultar_solicitud_mp_scae
        ...
        If (e.RowIndex >= 0) Then
            ...
            If (col = colVer.Name) Then
                ...
                Dim sql As String = "SELECT l.tipo,l.numero,l.cantidad,l.fec " & _
                                        "FROM J_salida_materia_prima_Tscae_transaccion t , CORSAN.dbo.documentos_lin l   " & _
            ...
            ElseIf (col = colBorrar.Name) Then
                ...
                If (motivo <> "") Then
                    ...
                    sql = "UPDATE J_salida_materia_prima_Tscae_enc SET anulado = 1,observaciones += '" & motivo & "' WHERE numero =" & numero
                    listSql.Add(sql)
    ...
    Private Sub cargarConsulta()
        ...
        End If
        selectSql = "SELECT E.numero,D.id_detalle,E.fecha,D.codigo,D.cantidad,(SELECT CASE WHEN (SELECT sum(peso) FROM J_salida_materia_prima_Tscae_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle ) is null THEN 0 ELSE D.cantidad-(D.cantidad -(SELECT sum(peso) FROM J_salida_materia_prima_Tscae_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle )) END ) As entregado_kilos,E.solicitante,t.nombres,E.observaciones " &
                       "FROM J_salida_materia_prima_Tscae_enc E ,J_salida_materia_prima_Tscae_det D, CORSAN.dbo.terceros t "
        ...
        If Not (chkTerminados.Checked) Then
            whereSql &= " AND (SELECT CASE WHEN (SELECT sum(peso) FROM J_salida_materia_prima_Tscae_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle ) is null THEN 0 ELSE (D.cantidad -(SELECT sum(peso) FROM J_salida_materia_prima_Tscae_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle )) END )<D.cantidad "
        End If
