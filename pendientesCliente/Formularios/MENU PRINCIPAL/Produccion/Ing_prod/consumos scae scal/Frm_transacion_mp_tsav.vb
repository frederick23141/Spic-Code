...
Imports accesoDatos
Public Class Frm_transacion_mp_tsav
    ...
    Private Sub cargar_solicitudes()
        ...
        Dim sql As String = "SELECT E.numero,D.id_detalle,E.fecha,D.codigo,(SELECT CASE WHEN (SELECT sum(peso) FROM J_salida_materia_prima_Tsav_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle ) is null THEN D.cantidad  ELSE (D.cantidad -(SELECT sum(peso) FROM J_salida_materia_prima_Tsav_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle )) END )As pendiente  ,R.descripcion FROM J_salida_materia_prima_Tsav_enc E ,J_salida_materia_prima_Tsav_det D, CORSAN.dbo.referencias R " & _
    ...
    Private Sub guardar()
        Using New Centered_MessageBox(Me)
            ...
            Next
            If peso <= stock Then
                listSql.AddRange(traslado_sav(codigo, peso, tipo, costo_unit))
                ...
                Dim sql As String = "update " & db_produccion & "J_rollos_tref set sav=" & numero_transaccion & " where  id_detalle =" & id_detalle & "  AND id_rollo =" & id_rollo & "and cod_orden=" & consecutivo_materia_prima & ""
                listSql.Add(sql)

                sql_solicitud = "INSERT INTO " & db_produccion & "J_salida_materia_prima_Tsav_transaccion" &
    ...
    Public Sub addRollo(ByVal consecutivo As String, ByVal id_detalle As String, ByVal num_rollo As String, ByVal tipo As String, ByVal peso As Double, ByVal cost_unitario As Double, ByVal codigo As String, ByVal descripcion As String)
        ...
        End Using
    End Sub
    Private Function traslado_sav(ByVal codigo As String, ByVal cantidad As Double, ByVal tipo As String, ByVal costo_unit As Double) As List(Of Object)
        Dim listSql As New List(Of Object)
        Dim dFec As Date = Now
        Dim usuario As String = nit_usuario
        Dim notas As String = "SPIC fecha:" & Now & " usuario:" & usuario
        numero_transaccion = objOrdenProdLn.mover_consecutivo(tipo)
        'UTiliza el metodo de la scal para tambien hacer las scae, que sirve para todos los consumos.
    ...
    Private Function validar_guardado_rollo(ByVal peso As String)
        ...
        Dim sql_rollo As String = "SELECT (SELECT CASE WHEN (SELECT sum(peso) FROM J_salida_materia_prima_Tsav_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle ) is null THEN D.cantidad  ELSE (D.cantidad -(SELECT sum(peso) FROM J_salida_materia_prima_Tsav_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle )) END )As pendiente FROM J_salida_materia_prima_Tsav_enc E ,J_salida_materia_prima_Tsav_det D, CORSAN.dbo.referencias R where E.numero=" & num_solicitud & " AND E.anulado is null AND  R.codigo = D.codigo AND (e.devolver = 'N' OR e.devolver IS NULL ) AND E.numero = D.numero AND id_detalle = D.id_detalle"
    ...
    Private Function validarTrasladoTref(ByVal consecutivo As String) As Boolean
        ...
        Dim id_detalle As String = obj_gestion_materia_primaLn.extraerDatoCodigoBarras("detalle", consecutivo)
        Dim id_rollo As String = obj_gestion_materia_primaLn.extraerDatoCodigoBarras("id_rollo", consecutivo)
        Dim sqlTraslado = "SELECT sav,sar,scal,anulado,traslado FROM j_rollos_tref  WHERE id_detalle =" & id_detalle & "  AND id_rollo =" & id_rollo & "and cod_orden=" & consecutivo_materia_prima & ""
        ...
        For i = 0 To dt_validacion.Rows.Count - 1
            If IsNumeric(dt_validacion.Rows(i).Item("sav")) Or IsNumeric(dt_validacion.Rows(i).Item("sar")) Or IsNumeric(dt_validacion.Rows(i).Item("scal")) Then
            ...
            If validarCodigoBarrasTref(consecutivo) Then
                ...
                If validarTrasladoTref(consecutivo) Then
                    ...
                    If validar_guardado_rollo(peso) Then
                        ...
                        Else
                            Using New Centered_MessageBox(Me)
                                MessageBox.Show("El código del tiquete no pertenece al pedido", "Código no pertenece al pedido", MessageBoxButtons.OK, MessageBoxIcon.Error)
                            End Using
                            leer_nuevo()
                        End If
                    Else
                        Using New Centered_MessageBox(Me)
                            MessageBox.Show("El tiquete excede el peso del pedido", "Tiquete excede el peso del pedido", MessageBoxButtons.OK, MessageBoxIcon.Error)
                        End Using
                    End If
                Else
                    leer_nuevo()
