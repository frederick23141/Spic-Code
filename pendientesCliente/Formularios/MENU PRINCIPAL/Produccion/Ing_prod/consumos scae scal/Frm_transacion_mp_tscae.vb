...
Imports accesoDatos
Public Class Frm_transacion_mp_tscae
    ...
    Private Sub guardar()
        Using New Centered_MessageBox(Me)
            ...
            Next
            If peso <= stock Then
                listSql.AddRange(traslado_scae(codigo, peso, tipo, costo_unit))
                ...
                Dim sql As String = "UPDATE " & db_produccion & " JB_rollos_rec SET scae =" & numero_transaccion & " WHERE cod_orden_rec =" & consecutivo_materia_prima & " AND id_rollo_rec = " & id_rollo & " AND id_detalle_rec = " & id_detalle
                listSql.Add(sql)
    ...
    Public Sub addRollo(ByVal consecutivo As String, ByVal id_detalle As String, ByVal num_rollo As String, ByVal tipo As String, ByVal peso As Double, ByVal cost_unitario As Double, ByVal codigo As String, ByVal descripcion As String)
        ...
        End Using
    End Sub
    Private Function traslado_scae(ByVal codigo As String, ByVal cantidad As Double, ByVal tipo As String, ByVal costo_unit As Double) As List(Of Object)
        Dim listSql As New List(Of Object)
        Dim dFec As Date = Now
        Dim usuario As String = nit_usuario
    ...
    Private Function validarCodigoBarrasRec(ByVal consecutivo As String) As Boolean
        ...
        If consecutivo_materia_prima <> "" And id_detalle <> "" And id_rollo <> "" Then
            Dim sql As String = "SELECT s.prod_final FROM JB_rollos_rec r, JB_orden_prod_rec_refs s " & _
                                 " WHERE (r.id_prof_final = s.num AND r.cod_orden_rec = s.cod_orden) " & _
    ...
    Private Function validarTrasladoRec(ByVal consecutivo As String) As Boolean
        ...
        Dim id_detalle As String = obj_gestion_materia_primaLn.extraerDatoCodigoBarras("detalle", consecutivo)
        Dim id_rollo As String = obj_gestion_materia_primaLn.extraerDatoCodigoBarras("id_rollo", consecutivo)
        Dim sqlTraslado As String = "SELECT r.tipo_trans,r.scae FROM JB_rollos_rec r, JB_orden_prod_rec_refs s " & _
                              " WHERE (r.id_prof_final = s.num AND r.cod_orden_rec = s.cod_orden) " & _
                                  " AND r.cod_orden_rec =" & consecutivo_materia_prima & " AND r.id_rollo_rec = " & id_rollo & " AND r.id_detalle_rec = " & id_detalle
        ...
        For i = 0 To dt_validacion.Rows.Count - 1
            If IsNumeric(dt_validacion.Rows(i).Item("scae")) Then
                MessageBox.Show("El rollo ya a sido consumido ", "Rollo Consumido", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Else
                If dt_validacion.Rows(i).Item("tipo_trans") <> "EPPP" Then
                    Using New Centered_MessageBox(Me)
                        MessageBox.Show("El rollo no es de bodega 2", "Rollo No es de bodega 2", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    End Using
                Else
                    resp = True
                End If
    ...
    Private Sub txtCodigoLector_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtCodigoLector.KeyPress
        If e.KeyChar = Microsoft.VisualBasic.ChrW(13) Then
            ...
            If validarCodigoBarrasRec(consecutivo) Then
                Dim consecutivo_materia_prima As String = obj_gestion_materia_primaLn.extraerDatoCodigoBarras("num_consecutivo", consecutivo)
                Dim id_detalle As String = obj_gestion_materia_primaLn.extraerDatoCodigoBarras("detalle", consecutivo)
                Dim id_rollo As String = obj_gestion_materia_primaLn.extraerDatoCodigoBarras("id_rollo", consecutivo)
                If validarTrasladoRec(consecutivo) Then
                    Dim sql As String = "SELECT s.prod_final AS codigo,r.peso AS cantidad FROM JB_rollos_rec r, JB_orden_prod_rec_refs s " & _
                              " WHERE (r.id_prof_final = s.num AND r.cod_orden_rec = s.cod_orden) " & _
                                  " AND r.cod_orden_rec =" & consecutivo_materia_prima & " AND r.id_rollo_rec = " & id_rollo & " AND r.id_detalle_rec = " & id_detalle
                    Dim dt As DataTable = objOpSimplesLn.listar_datatable(sql, "PRODUCCION")
                    Dim peso As String = dt.Rows(0).Item("cantidad")
                    Dim codigo As String = dt.Rows(0).Item("codigo")
