...
Imports accesoDatos
Public Class Frm_transacion_mp_tscal
    ...
    Private Sub cargar_solicitudes()
        ...
        Dim sql As String = "SELECT E.numero,D.id_detalle,E.fecha,D.codigo,(SELECT CASE WHEN (SELECT sum(peso) FROM J_salida_materia_prima_Tscal_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle ) is null THEN D.cantidad  ELSE (D.cantidad -(SELECT sum(peso) FROM J_salida_materia_prima_Tscal_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle )) END )As pendiente  ,R.descripcion FROM J_salida_materia_prima_Tscal_enc E ,J_salida_materia_prima_Tscal_det D, CORSAN.dbo.referencias R " & _
    ...
    Private Sub guardar()
        Using New Centered_MessageBox(Me)
            ...
            Next
            If peso <= stock Then
                listSql.AddRange(traslado_scal(codigo, peso, tipo, costo_unit))
                ...
                Dim sql As String = "update " & db_produccion & "J_rollos_tref set scal=" & numero_transaccion & " where  id_detalle =" & id_detalle & "  AND id_rollo =" & id_rollo & "and cod_orden=" & consecutivo_materia_prima & ""
                listSql.Add(sql)
    ...
    Public Sub addRollo(ByVal consecutivo As String, ByVal id_detalle As String, ByVal num_rollo As String, ByVal tipo As String, ByVal peso As Double, ByVal cost_unitario As Double, ByVal codigo As String, ByVal descripcion As String)
        ...
        End Using
    End Sub
    Private Function traslado_scal(ByVal codigo As String, ByVal cantidad As Double, ByVal tipo As String, ByVal costo_unit As Double) As List(Of Object)
        Dim listSql As New List(Of Object)
        Dim dFec As Date = Now
        Dim usuario As String = nit_usuario
    ...
    Private Function validar_guardado_rollo(ByVal peso As String)
        ...
        Dim sql_rollo As String = "SELECT (SELECT CASE WHEN (SELECT sum(peso) FROM J_salida_materia_prima_Tscal_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle ) is null THEN D.cantidad  ELSE (D.cantidad -(SELECT sum(peso) FROM J_salida_materia_prima_Tscal_transaccion  WHERE numero = D.numero AND id_detalle = D.id_detalle )) END )As pendiente FROM J_salida_materia_prima_Tscal_enc E ,J_salida_materia_prima_Tscal_det D, CORSAN.dbo.referencias R where E.numero=" & num_solicitud & " AND E.anulado is null AND  R.codigo = D.codigo AND (e.devolver = 'N' OR e.devolver IS NULL ) AND E.numero = D.numero AND id_detalle = D.id_detalle"
    ...
    Private Function validarTrasladoTref(ByVal consecutivo As String) As Boolean
        ...
        Dim id_detalle As String = obj_gestion_materia_primaLn.extraerDatoCodigoBarras("detalle", consecutivo)
        Dim id_rollo As String = obj_gestion_materia_primaLn.extraerDatoCodigoBarras("id_rollo", consecutivo)
        Dim sqlTraslado = "SELECT scal,sav,sar,anulado,traslado FROM j_rollos_tref  WHERE id_detalle =" & id_detalle & "  AND id_rollo =" & id_rollo & "and cod_orden=" & consecutivo_materia_prima & ""
        ...
        For i = 0 To dt_validacion.Rows.Count - 1
            ...
            If IsNumeric(dt_validacion.Rows(i).Item("scal")) Or IsNumeric(dt_validacion.Rows(i).Item("sav")) Or IsNumeric(dt_validacion.Rows(i).Item("sar")) Then
