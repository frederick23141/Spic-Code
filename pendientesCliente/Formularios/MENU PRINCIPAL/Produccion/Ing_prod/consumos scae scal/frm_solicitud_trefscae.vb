...
Public Class frm_solicitud_trefscae
    ...
    Private Sub guardar()
        ...
        End If
        sqlDet = "DELETE FROM J_salida_materia_prima_Tscae_det WHERE numero =" & numero
        listSql.Add(sqlDet)
        sqlEnc = "DELETE FROM J_salida_materia_prima_Tscae_enc WHERE numero =" & numero
        listSql.Add(sqlEnc)
        sqlEnc = "INSERT INTO J_salida_materia_prima_Tscae_enc (numero,fecha,solicitante,observaciones,devolver) " & _
        ...
        For i = 0 To dtg_solicitud.RowCount - 1
            If (dtg_solicitud.Item(colDesc.Name, i).Value <> "") Then
                sqlDet = "INSERT J_salida_materia_prima_Tscae_det (numero,id_detalle,codigo,cantidad) VALUES " & _
    ...
    Public Sub guardar_automatico(ByVal cod_rec As String, ByVal cantidad As String, ByVal mes As Integer, ByVal year As Integer)
        ...
        Dim observaciones As String = "Pedido automatico de scae desde el spic"
        Dim sqlDet As String = ""
        Dim sqlEnc As String = ""
        Dim medida As String = ""
        Dim sql As String = "SELECT max(d.numero) FROM J_salida_materia_prima_Tscae_enc e JOIN J_salida_materia_prima_Tscae_det d on e.numero=d.numero" & _
                            " where year(e.fecha)=" & Now.Year & " and month(e.fecha)=" & Now.Month & " and d.codigo='" & cod_rec & "'"
        ...
        If existe_orden = "" Then
            ...
            listSql.Add(sqlDet)
        Else
            sqlDet = "UPDATE J_salida_materia_prima_Tscae_det set cantidad=cantidad+" & cantidad & " where numero=" & existe_orden & ""
            listSql.Add(sqlDet)
        End If
        insertoCorrecto = objIngresoProdLn.ExecuteSqlTransaction(listSql, "PRODUCCION")
        If (insertoCorrecto) Then
