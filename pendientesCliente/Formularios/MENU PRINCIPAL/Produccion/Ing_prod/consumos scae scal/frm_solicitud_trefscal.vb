...
Public Class frm_solicitud_trefscal
    ...
    Private Sub guardar()
        ...
        End If
        If (numeroExistente = 0) Then
            numero = objIngresoProdLn.consultar_valor("SELECT MAX (numero)  FROM J_salida_materia_prima_Tscal_enc ", "PRODUCCION") + 1
        Else
            numero = numeroExistente
        End If
        sqlDet = "DELETE FROM J_salida_materia_prima_Tscal_det WHERE numero =" & numero
        listSql.Add(sqlDet)
        sqlEnc = "DELETE FROM J_salida_materia_prima_Tscal_enc WHERE numero =" & numero
        listSql.Add(sqlEnc)
        sqlEnc = "INSERT INTO J_salida_materia_prima_Tscal_enc (numero,fecha,solicitante,observaciones,devolver) " & _
        ...
        For i = 0 To dtg_solicitud.RowCount - 1
            If (dtg_solicitud.Item(colDesc.Name, i).Value <> "") Then
                sqlDet = "INSERT J_salida_materia_prima_Tscal_det (numero,id_detalle,codigo,cantidad) VALUES " & _
        ...
        End If
    End Sub
    Public Sub guardar_automatico(ByVal cod_rec As String, ByVal cantidad As String, ByVal mes As Integer, ByVal year As Integer)
        dtg_solicitud.CurrentCell = Nothing
        Dim insertoCorrecto As Boolean = False
        Dim listSql As New List(Of Object)
        Dim numero As Integer = 0
        ...
        Dim observaciones As String = "Pedido automatico de scal desde el spic"
        Dim sqlDet As String = ""
        Dim sqlEnc As String = ""
        Dim medida As String = ""
        Dim sql As String = "SELECT max(d.numero) FROM J_salida_materia_prima_Tscal_enc e JOIN J_salida_materia_prima_Tscal_det d on e.numero=d.numero" & _
        ...
        End If
        If (numeroExistente = 0) Then
            numero = objIngresoProdLn.consultar_valor("SELECT MAX (numero)  FROM J_salida_materia_prima_Tscae_enc ", "PRODUCCION") + 1
        Else
            numero = numeroExistente
        End If
        sqlDet = "DELETE FROM J_salida_materia_prima_Tscal_det WHERE numero =" & numero
        ...
        listSql.Add(sqlDet)
        Else
        sqlDet = "UPDATE J_salida_materia_prima_Tscal_det set cantidad=cantidad+" & cantidad & " where numero=" & existe_orden & ""
        listSql.Add(sqlDet)
        End If
        insertoCorrecto = objIngresoProdLn.ExecuteSqlTransaction(listSql, "PRODUCCION")
        If (insertoCorrecto) Then
